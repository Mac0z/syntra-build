...
class SQLiteProvisioningRepository:
    ...
    def ensure_intent(
        self, project_id: ProjectId, owner: str, name: str,
        visibility: RepositoryVisibility, occurred_at: datetime,
    ) -> GitHubRepository:
        existing = self.for_project(project_id)
        full_name = f"{owner}/{name}"
        if existing is not None:
            if (existing.owner, existing.repository_name, existing.full_name,
                existing.visibility) != (owner, name, full_name, visibility):
        ...
        try:
            self.connection.execute(
                ...
                (str(uuid4()), str(project_id), "github", owner, name, full_name,
                 visibility.value, occurred_at.isoformat(), occurred_at.isoformat()),
            )
        except sqlite3.Error as error:
            raise PersistenceError("repository intent could not be persisted") from error
    ...
    def for_project(self, project_id: ProjectId) -> GitHubRepository | None:
        ...
        return GitHubRepository(
            row["id"], project_id, row["provider"], row["owner"],
            row["repository_name"], row["full_name"], row["external_repository_id"],
            RepositoryVisibility(row["visibility"]), row["default_branch"],
            RepositoryProvisioningStatus(row["status"]),
            _required_time(row["created_at"]), _required_time(row["updated_at"]),
    ...
    def identify(
        self, repository_id: str, external_id: int, default_branch: str | None,
        ...
        try:
            changed = self.connection.execute(
                ...
                (external_id, default_branch, occurred_at.isoformat(), repository_id,
                 external_id),
            ).rowcount
        except sqlite3.Error as error:
            raise PersistenceError("repository identity could not be recorded") from error
        if changed != 1:
    ...
    def save_baseline(self, baseline: RepositoryBaseline) -> None:
        try:
            self.connection.execute(
                ...
                (baseline.id, str(baseline.project_id), baseline.github_repository_id,
                 baseline.commit_sha, str(baseline.spec_document_id),
                 baseline.spec_revision, baseline.spec_content_hash,
                 str(baseline.agents_document_id), baseline.agents_revision,
                 baseline.agents_content_hash, baseline.created_at.isoformat()),
            )
        except sqlite3.Error as error:
            raise PersistenceError("repository baseline could not be persisted") from error
    ...
    def baseline(self, project_id: ProjectId) -> RepositoryBaseline | None:
        ...
        return RepositoryBaseline(
            row["id"], project_id, row["github_repository_id"], row["commit_sha"],
            ProjectDocumentId.from_string(row["spec_document_id"]), row["spec_revision"],
            row["spec_content_hash"], ProjectDocumentId.from_string(row["agents_document_id"]),
            row["agents_revision"], row["agents_content_hash"],
            _required_time(row["created_at"]),
            _time(row["verified_at"]),
        )

    def verify(self, repository_id: str, baseline_id: str, occurred_at: datetime) -> None:
        try:
            self.connection.execute(
            ...
            self.connection.execute(
                ...
                WHERE id=?""", (occurred_at.isoformat(), occurred_at.isoformat(), repository_id),
            )
        except sqlite3.Error as error:
            raise PersistenceError("verification evidence could not be persisted") from error
