...
- document revisions increase independently for each project and document type;
...
- superseding an approved document and approving its replacement are one database
  transaction, backed by a partial unique index allowing at most one active approval;
